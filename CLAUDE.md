...
- **`ImmichDrive.ThumbnailProvider.dll`** (project `ImmichDrive.ThumbnailProvider/`) — an
  in-process COM **`IThumbnailProvider`** shell extension. Explorer loads it (in
  `dllhost.exe`) to render thumbnails for placeholder files **without hydrating** them: it
  maps the file path → Immich asset id via the shared on-disk index and fetches Immich's
  small thumbnail over HTTP. Must stay dependency-light and WinUI-free (it runs inside the
  shell). Registered via the MSIX manifest's COM extension.
...
1. **Populate** — `PlaceholderPopulator` walks the Immich timeline
   ...
   `CfCreatePlaceholders` to lay down 0-byte placeholders under the sync root:
   `Year\MM-Month\<date>_<name>.jpg`, plus a flat `Recent\` folder for the last
   `RecentDays` days. The Immich **asset id** is stored both as the placeholder's
...
| File | Role |
|---|---|
| `Models/ImmichAsset.cs` | Plain POCO: asset id, type, fileCreatedAt, originalFileName, checksum. |
...
| `Services/ImmichClient.cs` | `HttpClient` wrapper: `x-api-key` auth, timeline, original, thumbnail, ping. WinUI-free. |
...
- MSIX: `ImmichDriveMSIX/generate-msix-images.ps1` then `build-msix.ps1` (publishes the app
  + the extension DLL; `-NoSign` for Store). Set `<Identity>` from Partner Center first.
